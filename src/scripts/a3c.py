...
class Worker(mp.Process):
        def __init__(self, worker_id, learning_rate, env, device, queue, policy_kwargs, T_max=1000):
            super().__init__()
            self.worker_id = worker_id
            self.env = env
            # define lr schedule
            lr_schedule = self.constant_fn(learning_rate)
            self.policy = ActorCriticPolicy(env.observation_space, env.action_space, lr_schedule, **policy_kwargs)
            self.T_max = T_max
        ...
        def run(self):
            ...
            # define optimizer
            self.policy.optimizer = th.optim.RMSprop(self.policy.parameters(), lr=self.global_a3c.learning_rate)
            t_start = self.t
            done = False
            # get initial state
...
class A3C(OnPolicyAlgorithm):
    ...
    def __init__(
        ...
        if _init_setup_model:
            self._setup_model()
        self.optimizer = th.optim.RMSprop(self.policy.parameters(), lr=learning_rate)
    ...
    def train(self) -> None:
        ...
        worker_args = [(i, self.policy_class, self.env, self.learning_rate, self.device, self.queue, self.policy_kwargs) for i in range(n_workers)]

        while T < self._total_timesteps:
            # run workers
            results = pool.amap(worker_function, worker_args)

            # accumulate gradients using optimizer RMSprop
            self.optimizer.zero_grad()
            for d_theta_actor, d_theta_critic, worker_id, t, actor_loss, critic_loss in results:
