...
class RolloutBuffer(BaseBuffer):
    ...
    def add(
        ...
        self.values[self.pos] = np.array([value], dtype=int)
        # print("********type of value and log_prob", type(value), type(log_prob))
        self.log_probs[self.pos] = np.array([log_prob], dtype=int)
        self.pos += 1
        if self.pos == self.buffer_size:
            self.full = True
...
class DictRolloutBuffer(RolloutBuffer):
    ...
    def add(  # type: ignore[override]
        ...
        self.rewards[self.pos] = np.array(reward)
        self.episode_starts[self.pos] = np.array(episode_start)
        self.values[self.pos] = np.array(value, dtype=int)#.clone().cpu().numpy().flatten()
        self.log_probs[self.pos] = np.array(log_prob, dtype=int)#.clone().cpu().numpy()
        self.pos += 1
        if self.pos == self.buffer_size:
            self.full = True
